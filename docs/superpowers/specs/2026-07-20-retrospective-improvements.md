...
**Must-fix before calling this “ops-ready”:** Four P0 gaps can lose work or waste GitHub quota under failure:

1. **Rate-limit burns retries** — 429 / secondary rate limit counts toward `maxRetries` and can permanently fail jobs while the window is still closed; in-process sleep + republish also holds concurrency slots.
...
4. **Compose ships infra only** — no `Dockerfile` / api / worker services despite the implementation plan listing them; local demo still depends on host `make run-*`.
...
| Middleware order: RequestID → Logging → Recovery (so panics still carry request id) | Recovery → RequestID; **no** access logger | Reorder; add slog access log with `request_id` on every line ([Go slog](https://pkg.go.dev/log/slog), common Gin pattern) |
...
### P0 — correctness / data loss / quota

| ID | Item | Effort | Suggested approach |
|----|------|--------|-------------------|
| P0-1 | **Rate-limit must not burn retries** | M | Treat `KindRateLimited` as “wait, don’t consume attempt budget” (or separate budget). Honor global `github:rate_limit_until` before any GitHub call across all in-flight workers. Cap concurrent GitHub calls during backoff. Longer-term: move delay to TTL retry queue so sleep does not hold prefetch slots. |
...
| P0-4 | **Dockerfile + Compose api/worker** | M | Multi-stage Dockerfile (build both binaries). Compose services `api` / `worker` with `depends_on` healthchecks, env from `.env`, expose `8080`. Keep infra-only profile optional for host-run demos. |

### P1 — production hygiene

| ID | Item | Effort | Suggested approach |
|----|------|--------|-------------------|
| P1-1 | Middleware order + access logs | S | `RequestID` → slog access middleware (`request_id`, method, path, status, latency) → `Recovery`. |
