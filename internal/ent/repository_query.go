...
type RepositoryQuery struct {
	...
	withRefreshJobs *RefreshBatchJobQuery
...
// QueryRefreshJobs chains the current query on the "refresh_jobs" edge.
func (_q *RepositoryQuery) QueryRefreshJobs() *RefreshBatchJobQuery {
...
func (_q *RepositoryQuery) Clone() *RepositoryQuery {
	...
	return &RepositoryQuery{
		...
		withRefreshJobs: _q.withRefreshJobs.Clone(),
...
// WithRefreshJobs tells the query-builder to eager-load the nodes that are connected to
// the "refresh_jobs" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RepositoryQuery) WithRefreshJobs(opts ...func(*RefreshBatchJobQuery)) *RepositoryQuery {
	query := (&RefreshBatchJobClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withRefreshJobs = query
	return _q
}
...
func (_q *RepositoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Repository, error) {
	...
	if query := _q.withRefreshJobs; query != nil {
		if err := _q.loadRefreshJobs(ctx, query, nodes,
			func(n *Repository) { n.Edges.RefreshJobs = []*RefreshBatchJob{} },
			func(n *Repository, e *RefreshBatchJob) { n.Edges.RefreshJobs = append(n.Edges.RefreshJobs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RepositoryQuery) loadRefreshJobs(ctx context.Context, query *RefreshBatchJobQuery, nodes []*Repository, init func(*Repository), assign func(*Repository, *RefreshBatchJob)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(refreshbatchjob.FieldRepoID)
	}
	query.Where(predicate.RefreshBatchJob(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(repository.RefreshJobsColumn), fks...))
	}))
